...
class DownController extends Controller {
	...
	public function paged(){
		...
		$T=M('Down')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit($page*$size,$size)->select();
	...
	public function search(){
		...
		$count=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))->count();
		$T=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit($page*$size,$size)->select();	
		$json['pagecount']=ceil($count/$size);
		$json['pagecurrent']=$page;
		$json['data']['rows']=showitem($T);;
		$json['status']['err']=0;
	...
	public function del(){
		...
		if(M('Down')->where('id in('.I("post.ids","-1").')')->save($data)){ //删除成功后刷新数据
			$page=I("post.page",0);
			$size=I("post.size",5);
			$count=M('Down')->where("isdelete=0 and ver=".session("ver"))->count();
			$T=M('Down')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit($page*$size,$size)->select();	
			if($T){ //数据表有数据时
				$json['pagecount']=ceil($count/$size);
				$json['pagecurrent']=$page;
	...
	public function up(){
		...
		$T=M('Down')->where("id=".I("post.cid",0)." and ver=".session("ver"))->find();
		$T1=M('Down')->where("id=".I("post.pid",0)." and ver=".session("ver"))->find();	
		if(!$T||!$T1){
			$json['status']['err']=2;
	...
	public function pageup(){
		...
		$size=I("post.size",5);
		$count=M('Down')->where("isdelete=0 and ver=".session("ver"))->count();
		$T1=M('Down')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit($page*$size-1,1)->select();
		if($count==0){
			$json['status']['err']=2;
			$json['status']['msg']="数据错误，上移失败.";
			$this->ajaxReturn($json, 'json');
		...
		if(M('Down')->where('id ='.$T["id"])->save($data) && M('Down')->where('id ='.$T1[0]["id"])->save($data1)){
			$S->commit();
			$T2=M('Down')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit(($page-1)*$size,$size)->select();
			$json['pagecount']=ceil($count/$size);
	...
	public function searchup(){
		...
		$count=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))->count();
		$T1=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit($page*$size-1,1)->select();
		if($count==0){
			$json['status']['err']=2;
			$json['status']['msg']="数据错误，上移失败.";
			$this->ajaxReturn($json, 'json');
		...
		if(M('Down')->where('id ='.$T["id"])->save($data) && M('Down')->where('id ='.$T1[0]["id"])->save($data1)){
			$S->commit();
			$count=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))->count();
			$T2=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit(($page-1)*$size,$size)->select();	
			$json['pagecount']=ceil($count/$size);
			$json['pagecurrent']=$page-1;
			$json['data']['rows']=showitem($T2);;
			$json['status']['err']=0;
	...
	public function pagedown(){
		...
		$size=I("post.size",5);
		$count=M('Down')->where("isdelete=0 and ver=".session("ver"))->count();
		$T1=M('Down')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit(($page+1)*$size,1)->select();
		if($count==0){
			$json['status']['err']=2;
			$json['status']['msg']="数据错误，上移失败.";
			$this->ajaxReturn($json, 'json');
	...
	public function searchdown(){
		...
		$T1=M('Down')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit(($page+1)*$size,1)->select();	
		$data["orderid"]=$T1[0]["orderid"];
		$data1["orderid"]=$T["orderid"];
		$S->startTrans();
		if(M('Down')->where('id ='.$T["id"])->save($data) && M('Down')->where('id ='.$T1[0]["id"])->save($data1)){
	...
	public function AddSave(){
		...
		if($picname != ""){
        	...
        	$upload->savePath = '/Web/UploadFile/Down/img/'; // 设置附件上传（子）目录
		...
		if($upfile != ""){
			$upload->maxSize = 838800; // 设置附件上传大小
			$upload->exts = array('zip', 'rar', 'doc', 'docx'); // 设置附件上传类
        	$upload->savePath = '/Web/UploadFile/Down/file/'; // 设置附件上传（子）目录
       		$info = $upload->uploadOne($_FILES['upfile']);
			if(!$info) {// 上传错误提示错误信息
        		$json['status']['err']=2;
				$json['status']['msg']=$upload->getError();
				//$this->ajaxReturn($json, 'json');
				echo json_encode($json);
				exit;
    		}else{// 上传成功 获取上传文件信息
	...
	public function EditRead(){
		loadcheck(30);
		$info=M('Down')->where('id='.I("get.id",0).' and isdelete=0 and ver='.session("ver"))->find();
		if(!$info){
			header("Content-Type:text/html;charset=utf-8");
			echo "信息不存在!";
			exit;	
	...
	public function EditSave(){
		...
		$ret=M('Down')->where('id='.I('get.id',0).' and isdelete=0 and ver='.session("ver"))->find();
		if(!$ret){
			$json['status']['err']=2;
			$json['status']['msg']="信息提交有误！";
			//$this->ajaxReturn($json, 'json');
		...
		if($upfile!=""){
       		$upload->savePath = '/Web/UploadFile/Down/file/'; // 设置附件上传（子）目录
			$upload->exts = array('zip', 'rar', 'doc', 'xls'); // 设置附件上传类
        	$info = $upload->uploadOne($_FILES['upfile']);
			if($info) {// 上传错误提示错误信息
        		$data['upfile']=$info['savepath'].$info['savename'];
				$src=$_SERVER["DOCUMENT_ROOT"]. $ret["img"];
		...
		$data["putout"]=I('post.putout', '');
		$data["ver"]=session("ver");
		if(M('Down')->where('id='.I('get.id',0).' and isdelete=0 and ver='.session("ver"))->save($data)){
...
function showmenu($rid,$temp){
	$ret=M('deeptree')->where('parentid='.$rid.' and class="Down" and ver='.session("ver"))->order('orderid asc')->select();
	if(count($ret)){
		foreach($ret as $k=>$v){
			$str="";
			for($i=0;$i<$temp;$i++){
...
function showtree($rid){
	$ret=M('deeptree')->where('id='.$rid.' and class="Down" and ver='.session("ver"))->find();
	if($ret["parentid"]==0){
		return "-".$ret["id"];	
	}else{
