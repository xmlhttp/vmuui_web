...
class NetController extends Controller {
	...
	public function paged(){
		...
		$count=M('Net')->where("isdelete=0 and ver=".session("ver"))->count();
		$T=M('Net')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit($page*$size,$size)->select();
	...
	public function search(){
		...
		$count=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))->count();
		$T=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit($page*$size,$size)->select();	
		$json['pagecount']=ceil($count/$size);
		$json['pagecurrent']=$page;
		$json['data']['rows']=showitem($T);;
		$json['status']['err']=0;
	...
	public function edit(){
		...
		if($T){
			...
			login_info("【营销网络】 信息ID为[".I("post.rId",0). "] 更新[".$field."]成功", "Net");
			$json['status']['err']=0;
	...
	public function del(){
		...
		$data["isdelete"]=1;
		if(M('Net')->where('id in('.I("post.ids","-1").')')->save($data)){ //删除成功后刷新数据
			$page=I("post.page",0);
			$size=I("post.size",5);
			$count=M('Net')->where("isdelete=0 and ver=".session("ver"))->count();
			$T=M('Net')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit($page*$size,$size)->select();	
	...
	public function up(){
		...
		$T=M('Net')->where("id=".I("post.cid",0)." and ver=".session("ver"))->find();
		$T1=M('Net')->where("id=".I("post.pid",0)." and ver=".session("ver"))->find();	
		if(!$T||!$T1){
			$json['status']['err']=2;
		...
		$S->startTrans();
		if(M('Net')->where('id ='.$T["id"])->save($data) && M('Net')->where('id ='.$T1["id"])->save($data1)){
			$S->commit();
			$json['status']['err']=0;
	...
	public function pageup(){
		...
		$size=I("post.size",5);
		$count=M('Net')->where("isdelete=0 and ver=".session("ver"))->count();
		$T1=M('Net')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit($page*$size-1,1)->select();
		if($count==0){
			$json['status']['err']=2;
			$json['status']['msg']="数据错误，上移失败.";
			$this->ajaxReturn($json, 'json');
		...
		$data1["orderid"]=$T["orderid"];
		$S->startTrans();
		if(M('Net')->where('id ='.$T["id"])->save($data) && M('Net')->where('id ='.$T1[0]["id"])->save($data1)){
			$S->commit();
			$T2=M('Net')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit(($page-1)*$size,$size)->select();
			$json['pagecount']=ceil($count/$size);
	...
	public function searchup(){
		...
		$count=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))->count();
		$T1=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit($page*$size-1,1)->select();
		if($count==0){
			$json['status']['err']=2;
			$json['status']['msg']="数据错误，上移失败.";
			$this->ajaxReturn($json, 'json');
		...
		if(M('Net')->where('id ='.$T["id"])->save($data) && M('Net')->where('id ='.$T1[0]["id"])->save($data1)){
			$S->commit();
			$count=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))->count();
			$T2=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit(($page-1)*$size,$size)->select();	
			$json['pagecount']=ceil($count/$size);
			$json['pagecurrent']=$page-1;
			$json['data']['rows']=showitem($T2);;
			$json['status']['err']=0;
	...
	public function pagedown(){
		...
		$size=I("post.size",5);
		$count=M('Net')->where("isdelete=0 and ver=".session("ver"))->count();
		$T1=M('Net')->where("isdelete=0 and ver=".session("ver"))-> order('orderid desc')->limit(($page+1)*$size,1)->select();
		if($count==0){
			$json['status']['err']=2;
			$json['status']['msg']="数据错误，上移失败.";
			$this->ajaxReturn($json, 'json');
	...
	public function searchdown(){
		...
		$T1=M('Net')->where("isdelete=0".$str." and newtitle like '%".I("post.searchtxt",'')."%' and ver=".session("ver"))-> order('orderid desc')->limit(($page+1)*$size,1)->select();	
		$data["orderid"]=$T1[0]["orderid"];
		$data1["orderid"]=$T["orderid"];
		$S->startTrans();
		if(M('Net')->where('id ='.$T["id"])->save($data) && M('Net')->where('id ='.$T1[0]["id"])->save($data1)){
	...
	public function AddSave(){
		...
		if($picname != ""||$picname1 != ""){
       	...
       	if($picname != ""){
			$upload->savePath = '/Web/UploadFile/Net/Small/'; // 设置附件上传（子）目录
		...
		if($picname1 != ""){
			$upload->savePath = '/Web/UploadFile/Net/Big/'; // 设置附件上传（子）目录
		...
		if($lastInsId =M('Net')->add($data)){
			$data['orderid']=$lastInsId;
			if(M('Net')->where('id='.$lastInsId)->save($data)){
				login_info("【营销网络】 信息ID为[".$lastInsId."]的项添加成功", "Net");
				$json['status']['err']=0;
	...
	public function EditRead(){
		loadcheck(26);
		$info=M('Net')->where('id='.I("get.id",0).' and isdelete=0 and ver='.session("ver"))->find();
		if(!$info){
			header("Content-Type:text/html;charset=utf-8");
			echo "信息不存在!";
			exit;	
	...
	public function EditSave(){
		...
		$ret=M('Net')->where('id='.I('get.id',0).' and isdelete=0 and ver='.session("ver"))->find();
		if(!$ret){
			$json['status']['err']=2;
			$json['status']['msg']="信息提交有误！";
			//$this->ajaxReturn($json, 'json');
		...
		if(M('Net')->where('id='.I('get.id',0).' and isdelete=0 and ver='.session("ver"))->save($data)){
			login_info("【营销网络】 信息ID为[".I('get.id',0)."]的项修改成功", "Net");
			$json['status']['err']=0;
	...
	public function Swfupload(){
		...
		$upload->autoSub = true;
		$upload->subName = array('date','Y-m-d');
        $upload->savePath = '/Web/UploadFile/Net/pl/'; // 设置附件上传（子）目录
        $info = $upload->upload();
        if (!$info) {// 上传错误提示错误信息
...
function showitem($T){
	...
	foreach($T as $t=>$v){
		...
		$data[$t]["data"][]="编辑^/System.php?s=/System/Net/EditRead&id=".$v['id']."^_self";
		$data[$t]["data"][]=0;
	}
	return $data;
}
...
function showmenu($rid,$temp){
	$ret=M('deeptree')->where('parentid='.$rid.' and class="Net" and ver='.session("ver"))->order('orderid asc')->select();
	if(count($ret)){
		foreach($ret as $k=>$v){
			$str="";
			for($i=0;$i<$temp;$i++){
...
function showtree($rid){
	$ret=M('deeptree')->where('id='.$rid.' and class="Net" and ver='.session("ver"))->find();
	if($ret["parentid"]==0){
		return "-".$ret["id"];	
	}else{
